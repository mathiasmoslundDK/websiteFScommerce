...
(() => {
  ...
  // Standalone-bundle support: super_inline_html can only inline resources it
  // finds in HTML attributes it knows, so the sidecar fetch and any <image-slot
  // src> are invisible to it. A bundled page declares them as
  // <meta name="ext-resource-dependency" content="<url>" data-resource-id="<id>">
  // and the bundler exposes blob URLs on window.__resources. Outside a bundle
  // window.__resources is undefined and both helpers are no-ops.
  let resMap = null;
  const resolveUrl = (u) => {
    if (!u || !window.__resources) return u;
    if (!resMap) {
      resMap = {};
      try {
        document
          .querySelectorAll('meta[name="ext-resource-dependency"][data-resource-id]')
          .forEach((m) => {
            const orig = m.getAttribute('content');
            const mapped = window.__resources[m.getAttribute('data-resource-id')];
            if (orig && mapped) resMap[orig] = mapped;
          });
      } catch (e) { /* no metas — leave the map empty */ }
      // The bundler STRIPS the meta tags after harvesting them, so a bundled
      // page also declares window.__slotResMap = { "<url>": "<resource-id>" }
      // before support.js. That table survives and is the reliable path.
      try {
        const tbl = window.__slotResMap;
        if (tbl) Object.keys(tbl).forEach((k) => {
          const mapped = window.__resources[tbl[k]];
          if (mapped) resMap[k] = mapped;
        });
      } catch (e) { /* no table — metas may still have worked */ }
    }
    return resMap[u] || u;
  };
  const stateFileUrl = () =>
    (window.__resources && window.__resources.slotState) || STATE_FILE;
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      const srcAttr = resolveUrl(this.getAttribute('src') || '');
